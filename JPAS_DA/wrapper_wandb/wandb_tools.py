...
def wandb_train(
        config=None,
        aux_config_path = "/home/dlopez/Documents/Projects/JPAS_Domain_Adaptation/configs/aux_config_wandb_fixed_params_and_model_options.yaml"
    ):
    """
    ...
    with wandb.init(config=config) as run:
        ...
        log_path = os.path.join(merged_config["training"]["path_save"], 'register_sweeps.txt')
        logging.info(f"📄 Logging results to: {log_path}")
        with open(log_path, 'a') as ff:
            ff.write(run.name + ' ' + str(min_val_loss) + '\n')
